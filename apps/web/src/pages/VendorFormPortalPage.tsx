...
import { CheckCircle2, Clock3, FileText, Send, ShieldCheck } from 'lucide-react';
...
export function VendorFormPortalPage() {
  ...
  if (submitted) return <div className="life-public flex min-h-screen items-center justify-center p-4"><Card className="public-sheet max-w-xl"><CardBody className="py-12 text-center"><CheckCircle2 className="mx-auto h-16 w-16 text-emerald-500" /><h1 className="mt-4 text-2xl font-extrabold text-slate-900">ส่งผลการประเมินเรียบร้อยแล้ว</h1><p className="mt-2 text-slate-500">คำตอบของท่านถูกบันทึกใน {data.form_no} และแจ้งกลับไปยังทีม IT แล้ว</p></CardBody></Card></div>;

  return <div className="life-public min-h-screen pb-12">
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-4"><PublicBrand subtitle="Vendor Response Portal" /><div className="ml-auto hidden items-center gap-2 text-xs font-semibold text-emerald-700 sm:flex"><ShieldCheck className="h-4 w-4" />ลิงก์เฉพาะงาน · เข้ารหัสและมีวันหมดอายุ</div></div></header>
    <main className="mx-auto max-w-7xl space-y-4 px-4 py-6">
      <Card className="public-sheet"><CardBody className="grid gap-4 md:grid-cols-[1fr_auto]"><div><p className="text-xs font-bold uppercase tracking-wide text-primary-700">{data.form_no}</p><h1 className="mt-1 text-xl font-extrabold text-slate-900">{data.title}</h1><p className="mt-1 text-sm text-slate-500">{data.template?.name}{data.ticket ? ` · ${data.ticket.ticket_no} ${data.ticket.title}` : ''}</p></div><div className="rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-900"><p className="font-bold">กำหนดตอบกลับ</p><p>{data.vendor_due_at ? formatThaiDate(data.vendor_due_at) : 'ไม่ได้กำหนด'}</p></div></CardBody></Card>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_460px]">
        ...
        <Card className="public-sheet h-fit xl:sticky xl:top-4"><CardHeader><p>ส่วนตอบกลับโดย Vendor / Outsource</p><p className="mt-1 text-xs font-normal text-slate-500">กรอก SLA, Root Cause, วิธีแก้ และ Manday/Credit ให้ครบก่อนส่ง</p></CardHeader><CardBody>
