 ...
 *
 * ปุ่มส่งไป Google Sheets ต่อท้ายมาจากที่นี่ด้วย เพื่อให้ทุกหน้าที่มีปุ่มส่งออกอยู่แล้วได้ปลายทาง
 * เพิ่มโดยไม่ต้องแก้ทีละหน้า — และใช้ getRows ก้อนเดียวกัน ข้อมูลสองปลายทางจึงตรงกันเสมอ
 */
export function ExportCsvButton({ getRows, fileName, disabled = false, label = 'ส่งออกหน้านี้', className }: ExportCsvButtonProps) {
  return (
    <>
      ...
      <ExportSheetsButton disabled={disabled} getExport={() => ({ csv: toCsv(getRows()), fileName })} />
    </>
  );
}
