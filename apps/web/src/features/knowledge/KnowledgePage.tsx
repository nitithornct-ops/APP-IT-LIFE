...
function ArticleCard({ article, canManage, onOpen, onEdit }: { article: KnowledgeArticle; canManage: boolean; onOpen: () => void; onEdit: () => void }) {
  ...
  const deleteMutation = useMutation({ mutationFn: () => apiFetch(`/api/v1/knowledge/articles/${article.id}`, { method: 'DELETE' }), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['knowledge'] }) });
  return <Card data-testid={`knowledge-card-${article.id}`} className="flex h-full flex-col"><CardBody className="flex flex-1 flex-col">
    ...
    <div className="mt-4 flex flex-wrap items-center gap-2"><span className="text-xs text-slate-400"><Eye className="mr-1 inline h-3.5 w-3.5" />{article.views_count} · <ThumbsUp className="mr-1 inline h-3.5 w-3.5" />{article.helpful_count}</span><div className="ml-auto flex gap-1"><Button size="sm" onClick={onOpen}>อ่าน</Button>{canManage && <><Button size="sm" variant="outline" aria-label="แก้ไข" onClick={onEdit}><Pencil className="h-4 w-4" /></Button><Button size="sm" variant="outline" aria-label={article.status === 'เผยแพร่' ? 'เก็บเป็นร่าง' : 'เผยแพร่'} isLoading={statusMutation.isPending} onClick={() => statusMutation.mutate()}>{article.status === 'เผยแพร่' ? <FilePenLine className="h-4 w-4" /> : <Send className="h-4 w-4" />}</Button><Button size="sm" variant="ghost" aria-label="ลบบทความ" isLoading={deleteMutation.isPending} onClick={() => { if (window.confirm(`ลบบทความ “${article.title}” หรือไม่`)) deleteMutation.mutate(); }}><Trash2 className="h-4 w-4 text-red-600" /></Button></>}</div></div>
  </CardBody></Card>;
}
...
export function KnowledgePage() {
  ...
  async function openArticle(article: KnowledgeArticle) { try { setOpenError(null); const data = await apiFetch<KnowledgeArticle>(`/api/v1/knowledge/articles/${article.id}/view`, { method: 'POST' }); setSelected(data); void queryClient.invalidateQueries({ queryKey: ['knowledge'] }); } catch (reason) { setOpenError(errorText(reason, 'เปิดบทความไม่สำเร็จ')); } }
  ...
  return <div className="space-y-5" data-testid="knowledge-page">
    ...
    {showForm && <ArticleForm article={editing} overview={query.data} onClose={() => { setShowForm(false); setEditing(undefined); }} />}
    {selected && <ArticleDetail key={selected.id} article={selected} canFeedback={canFeedback && selected.status === 'เผยแพร่'} onClose={() => setSelected(undefined)} />}
