...
function ApprovalRow({ approval, instance }: { approval: WorkflowApproval; instance?: WorkflowInstance }) {
  ...
  const mutation = useMutation({ mutationFn: ({ decision, comment }: { decision: string; comment: string }) => apiFetch(`/api/v1/workflows/approvals/${approval.id}/decision`, { method: 'POST', body: JSON.stringify({ decision, comment }) }), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['workflows'] }), onError: (reason) => setError(errorText(reason, 'บันทึกคำตัดสินไม่สำเร็จ')) });
  function decide(decision: 'APPROVE' | 'REJECT' | 'RETURN') { const comment = window.prompt(decision === 'APPROVE' ? 'ความเห็นประกอบ (ไม่บังคับ)' : 'กรุณาระบุเหตุผล') ?? ''; if (decision !== 'APPROVE' && !comment.trim()) return; setError(null); mutation.mutate({ decision, comment }); }
  const overdue = workflowIsOverdue(approval.due_at, approval.status);
  return <tr data-testid={`workflow-approval-${approval.id}`} className="border-t border-slate-100 dark:border-slate-700"><td className="px-4 py-3"><p className="font-semibold">{instance?.record_label ?? instance?.record_id ?? approval.instance_id}</p><p className="text-xs text-slate-400">ขั้น {approval.step_order} · {instance?.module_key ?? 'workflow'}</p>{error && <p className="mt-1 text-xs text-red-600">{error}</p>}</td><td className="px-4 py-3"><Badge variant={statusTone(approval.status)}>{approval.status}</Badge></td><td className={`px-4 py-3 text-sm ${overdue ? 'font-semibold text-red-600' : ''}`}>{dateTime(approval.due_at)}{overdue && <p className="text-xs">เกินกำหนด</p>}</td><td className="px-4 py-3"><div className="flex flex-wrap gap-1">{approval.can_act && <><Button size="sm" onClick={() => decide('APPROVE')} isLoading={mutation.isPending}>อนุมัติ</Button><Button size="sm" variant="danger" onClick={() => decide('REJECT')}>ปฏิเสธ</Button><Button size="sm" variant="outline" onClick={() => decide('RETURN')}>ส่งกลับ</Button></>}</div></td></tr>;
}
...
export function WorkflowsPage() {
  ...
  return <div className="space-y-5" data-testid="workflows-page">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">Workflow / งานอนุมัติ</h1><p className="mt-1 text-sm text-slate-500">Engine กลางสำหรับอนุมัติหลายขั้น มอบหมายแทน และตรวจสอบย้อนหลัง</p></div><div className="flex flex-wrap gap-2">{canDelegate && <Button variant="outline" onClick={() => setShowDelegation(true)}><UserRoundCog className="h-4 w-4" />ตั้งผู้อนุมัติแทน</Button>}{canManage && <><Button variant="outline" onClick={() => setShowStart(true)}><Play className="h-4 w-4" />เริ่ม Workflow</Button><Button data-testid="workflow-definition-toggle" onClick={() => { setEditingDefinition(undefined); setShowDefinition(true); }}><Plus className="h-4 w-4" />สร้างแบบ</Button></>}</div></div>
    ...
    {showDefinition && optionsQuery.data && <DefinitionForm options={optionsQuery.data} definition={editingDefinition} onClose={() => { setShowDefinition(false); setEditingDefinition(undefined); }} />}
    {showStart && <StartForm definitions={data.definitions} onClose={() => setShowStart(false)} />}
    {showDelegation && optionsQuery.data && <DelegationForm options={optionsQuery.data} definitions={data.definitions} onClose={() => setShowDelegation(false)} />}
    ...
    {tab === 'approvals' && <Card><CardHeader>กล่องงานอนุมัติ</CardHeader><div className="overflow-x-auto">{data.myApprovals.length ? <table className="w-full min-w-[760px] text-left"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900/40"><tr><th className="px-4 py-3">รายการ</th><th className="px-4 py-3">สถานะ</th><th className="px-4 py-3">กำหนด</th><th className="px-4 py-3">ดำเนินการ</th></tr></thead><tbody>{data.myApprovals.map((approval) => <ApprovalRow key={approval.id} approval={approval} instance={instanceById.get(approval.instance_id)} />)}</tbody></table> : <EmptyState icon={<CheckCircle2 className="h-8 w-8" />} title="ไม่มีงานรออนุมัติ" description="งานที่ถูกมอบหมายให้คุณจะแสดงที่นี่" />}</div></Card>}
    {tab === 'instances' && <Card><CardHeader>Workflow Instances</CardHeader><div className="overflow-x-auto">{data.instances.length ? <table className="w-full min-w-[820px] text-left"><thead className="bg-slate-50 text-xs uppercase text-slate-500 dark:bg-slate-900/40"><tr><th className="px-4 py-3">รายการ</th><th className="px-4 py-3">Workflow</th><th className="px-4 py-3">สถานะ</th><th className="px-4 py-3">ความคืบหน้า</th><th className="px-4 py-3"></th></tr></thead><tbody>{data.instances.map((item) => <InstanceRow key={item.id} instance={item} />)}</tbody></table> : <EmptyState icon={<GitBranch className="h-8 w-8" />} title="ยังไม่มี Workflow" description="เริ่ม Workflow จากแบบที่เปิดใช้งาน" />}</div></Card>}
