...
export function IncidentDetailPage() {
  ...
  return <div className="flex flex-col gap-4" data-testid="incident-detail-page">
    ...
    <div className="flex flex-wrap items-start justify-between gap-2"><PageTitle eyebrow={`บริการและกระบวนการ IT / ${incident.incident_number}`} title={<>{incident.title}</>} description={<>รายงานโดย {incident.reporter?.full_name ?? incident.reporter?.email ?? '—'} · {formatThaiDate(incident.report_date, 'd MMM yyyy HH:mm')}</>} /><div className="flex flex-wrap gap-1"><Badge variant={incidentStatusTone[incident.status]}>{incident.status}</Badge>{incident.severity && <Badge variant={riskTone[incident.severity]}>{incident.severity}</Badge>}{incident.risk_level && <Badge variant={riskTone[incident.risk_level]}>Risk {incident.risk_level} ({incident.risk_score})</Badge>}{incident.contains_personal_data && <Badge variant="danger">ข้อมูลส่วนบุคคล</Badge>}</div></div>
