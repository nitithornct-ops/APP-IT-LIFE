...
export function ProblemDetailPage() {
  ...
  return <div className="flex flex-col gap-4" data-testid="problem-detail-page">
    ...
    <div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-mono text-sm text-slate-500">{problem.problem_number}</p><h1 className="text-xl font-bold">{problem.title}</h1><p className="text-sm text-slate-500">สร้างเมื่อ {formatThaiDate(problem.created_at, 'd MMM yyyy HH:mm')}</p></div><div className="flex gap-1"><Badge variant={priorityTone[problem.priority]}>{problem.priority}</Badge><Badge variant={problemStatusTone[problem.status]}>{problem.status}</Badge></div></div>
