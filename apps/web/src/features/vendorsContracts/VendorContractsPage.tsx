...
import { Building2, CalendarClock, ClipboardCheck, FileText, Loader2, Pencil, Plus, RefreshCw, X } from 'lucide-react';
...
export function VendorContractsPage() {
  ...
  return <div className="flex flex-col gap-4" data-testid="vendor-contracts-page">
    ...
    {showForm && tab === 'vendors' && (vendorRefs.isLoading ? <Loader2 className="mx-auto h-5 w-5 animate-spin" /> : vendorRefs.data && <VendorForm vendor={editingVendor} references={vendorRefs.data} onClose={resetForm} />)}
    {showForm && tab === 'contracts' && (contractRefs.isLoading ? <Loader2 className="mx-auto h-5 w-5 animate-spin" /> : contractRefs.data && <ContractForm contract={editingContract} references={contractRefs.data} onClose={resetForm} />)}
