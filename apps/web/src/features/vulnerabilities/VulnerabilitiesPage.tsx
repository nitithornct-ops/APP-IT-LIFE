...
export function VulnerabilitiesPage() {
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="vulnerabilities-page">
      ...
      {showForm && (optionsQuery.isLoading ? <Card><CardBody className="flex items-center justify-center gap-2 py-8 text-sm text-slate-500"><Loader2 className="h-5 w-5 animate-spin" />กำลังเตรียม Asset, CI และ Owner</CardBody></Card> : optionsQuery.isError || !optionsQuery.data ? <Card><CardBody className="flex items-center justify-between text-sm text-red-700"><span>โหลดตัวเลือกสำหรับแบบฟอร์มไม่สำเร็จ</span><Button size="sm" variant="ghost" onClick={resetForm}>ปิด</Button></CardBody></Card> : <FindingForm finding={editing} options={optionsQuery.data} currentUserId={me?.profile.id} onClose={resetForm} />)}
