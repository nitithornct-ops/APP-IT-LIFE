...
export function ChangeDetailPage() {
  ...
  return <div className="flex flex-col gap-4" data-testid="change-detail-page">
    {change.configuration_item && <Card data-testid="change-ci-link"><CardHeader>Configuration Item (CI)</CardHeader><CardBody><Link to={`/cmdb/${change.configuration_item.id}`} className="text-sm font-semibold text-primary-700 hover:underline dark:text-primary-300">{change.configuration_item.ci_code} — {change.configuration_item.name}</Link><p className="mt-1 text-xs text-slate-500">{change.configuration_item.ci_type} · {change.configuration_item.environment} · {change.configuration_item.criticality}</p></CardBody></Card>}
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <Card className="lg:col-span-2"><CardHeader>รายละเอียดคำขอ</CardHeader><CardBody className="grid gap-4 sm:grid-cols-2"><Info label="รายละเอียด">{change.description}</Info><Info label="ประเมินผลกระทบ">{change.impact_assessment}</Info><Info label="ประเภท Change">{change.change_type}</Info><Info label="แผน Rollback">{change.rollback_plan}</Info><Info label="Incident ต้นทาง">{change.source_incident ? <Link to={`/incidents/${change.source_incident.id}`} className="text-primary-700 hover:underline">{change.source_incident.incident_number} — {change.source_incident.title}</Link> : '—'}</Info><Info label="คำขอบริการต้นทาง">{change.source_service_request ? <Link to={`/service-requests/${change.source_service_request.id}`} className="text-primary-700 hover:underline">{change.source_service_request.service_code} — {change.source_service_request.service_name}</Link> : '—'}</Info><Info label="หมายเหตุ">{change.notes}</Info></CardBody></Card>
