...
export function AssetBorrowPage() {
  const [view, setView] = useState<BorrowView>('active');
  ...
  return (
    <div className="flex flex-col gap-5" data-testid="asset-borrow-page">
      ...
      <div className="flex gap-2 border-b border-slate-200 px-2 dark:border-slate-700">
        <button type="button" onClick={() => { setView('active'); setPage(1); }} className={`border-b-2 px-4 py-3 text-sm font-semibold ${view === 'active' ? 'border-primary-600 text-primary-700 dark:text-primary-300' : 'border-transparent text-slate-500'}`}>กำลังถือครอง / ค้างคืน ({summary.active})</button>
        <button type="button" onClick={() => { setView('history'); setPage(1); }} className={`border-b-2 px-4 py-3 text-sm font-semibold ${view === 'history' ? 'border-primary-600 text-primary-700 dark:text-primary-300' : 'border-transparent text-slate-500'}`}>ประวัติเคลื่อนไหว</button>
      </div>
        ...
        <CardBody>
          <div className="mb-4 flex flex-wrap gap-2 rounded-xl bg-slate-50 p-3 dark:bg-slate-900/40">
            <label className="relative min-w-64 flex-1">
              ...
              <input type="search" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="ค้นหาในรายการ..." className={`${fieldClass} pl-9`} />
            </label>
            <select value={departmentId} onChange={(event) => { setDepartmentId(event.target.value); setPage(1); }} className={`${fieldClass} w-auto min-w-48`}>
          ...
          {view === 'active' && activeRecords.length > 0 && (
            <div className="overflow-x-auto"><DataTable pagination={false} className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500 dark:bg-slate-900/40"><tr><th className="px-3 py-3">Asset</th><th className="px-3 py-3">ผู้ถือครอง</th><th className="px-3 py-3">แผนก / สถานที่</th><th className="px-3 py-3">ยืมเมื่อ</th><th className="px-3 py-3">กำหนดคืน</th></tr></thead><tbody>{activeRecords.map((item) => { const due = dueState(item.loan_due_date); return <tr key={item.id} className="border-t border-slate-100 dark:border-slate-700"><td className="px-3 py-3"><Link to={`/assets/${item.id}`} className="font-semibold text-primary-700 hover:underline dark:text-primary-300">{item.name}</Link><p className="font-mono text-xs text-slate-400">{item.asset_code}</p></td><td className="px-3 py-3">{fullName(item.owner)}{item.owner?.employee_code && <p className="text-xs text-slate-400">{item.owner.employee_code}</p>}</td><td className="px-3 py-3 text-slate-500">{item.department?.name_th ?? '—'}<p className="text-xs">{item.location ?? ''}</p></td><td className="px-3 py-3 text-slate-500">{item.loan_date ? formatThaiDate(item.loan_date, 'd MMM yyyy') : '—'}</td><td className="px-3 py-3"><Badge variant={due.variant}>{due.label}</Badge></td></tr>; })}</tbody></DataTable></div>
          )}
