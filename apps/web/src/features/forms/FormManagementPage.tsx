...
import { FormEvidencePanel } from './FormEvidencePanel';
import { StructuredFieldPanel } from './StructuredFieldPanel';
import { GenericLinkPanel } from './GenericLinkPanel';
import { TemplateGovernancePanel } from './TemplateGovernancePanel';
...
function downloadPdfBase64(pdfBase64: string, fileName: string): void {
...
const defaultAcknowledgementConfig: import('../../types/forms').FormAcknowledgementConfig = { enabled: false, statement: '' };
const defaultApprovalSignatureConfig: import('../../types/forms').FormApprovalSignatureConfig = { requiredRoles: [] };
const defaultDocumentNumberRule: import('../../types/forms').FormDocumentNumberRule = { prefix: 'FRM', dateFormat: 'YYYYMM', padding: 5 };
...
interface EditorState {
  ...
  html: string;
  originalHtml: string;
  fieldSchema?: import('../../types/forms').FormFieldDefinition[];
  originalFieldSchema?: import('../../types/forms').FormFieldDefinition[];
  acknowledgementConfig?: import('../../types/forms').FormAcknowledgementConfig;
  originalAcknowledgementConfig?: import('../../types/forms').FormAcknowledgementConfig;
  approvalSignatureConfig?: import('../../types/forms').FormApprovalSignatureConfig;
  originalApprovalSignatureConfig?: import('../../types/forms').FormApprovalSignatureConfig;
  documentNumberRule?: import('../../types/forms').FormDocumentNumberRule;
  originalDocumentNumberRule?: import('../../types/forms').FormDocumentNumberRule;
  formData?: Record<string, unknown>;
  originalFormData?: Record<string, unknown>;
  sourceModule?: FormModuleKey;
  sourceRecordId?: string;
  originalSourceModule?: FormModuleKey;
  originalSourceRecordId?: string;
...
export function FormManagementPage() {
  ...
  const canExport = hasPermission('form.export');
  const canSendVendor = hasPermission('form.vendor_send');
  const canClose = hasPermission('form.close');
  const [searchParams, setSearchParams] = useSearchParams();
  const [tab, setTab] = useState<'issues' | 'templates'>('templates');
  const [search, setSearch] = useState('');
  const [editor, setEditor] = useState<EditorState>();
  const [showNewTemplate, setShowNewTemplate] = useState(false);
  const [showImportGoogleDoc, setShowImportGoogleDoc] = useState(false);
  ...
  const [compareResult, setCompareResult] = useState<{ from: { version: number; content_html: string }; to: { version: number; content_html: string }; changed: Record<string, boolean> }>();
  const [newTemplate, setNewTemplate] = useState({ name: '', description: '', category: 'IT Support', moduleKey: '' as FormModuleKey | '', contentHtml: blankTemplate, fieldSchema: [] as import('../../types/forms').FormFieldDefinition[] });
  const [googleDocImport, setGoogleDocImport] = useState({ source: '', name: '', description: '', category: 'IT Support', moduleKey: '' as FormModuleKey | '' });
  const [newIssue, setNewIssue] = useState({ title: '', templateId: '', ticketId: '', sourceModule: 'ticket' as FormModuleKey, sourceRecordId: '' });
  ...
  // Keep compatibility with older API fixtures while hydrating the structured layer when it exists.
  useEffect(() => {
    if (!editor || editor.fieldSchema) return;
    const template = editor.kind === 'template' ? templatesQuery.data?.find((item) => item.id === editor.id) : undefined;
    const issue = editor.kind === 'issue' ? issuesQuery.data?.find((item) => item.id === editor.id) : undefined;
    const fieldSchema = template?.field_schema ?? issue?.field_schema ?? [];
    const formData = issue?.form_data ?? {};
    const templateConfig = template ?? issue?.template;
    const acknowledgementConfig = templateConfig?.acknowledgement_config ?? defaultAcknowledgementConfig;
    const approvalSignatureConfig = templateConfig?.approval_signature_config ?? defaultApprovalSignatureConfig;
    const documentNumberRule = templateConfig?.document_number_rule ?? defaultDocumentNumberRule;
    setEditor((current) => current && !current.fieldSchema ? { ...current, fieldSchema, originalFieldSchema: fieldSchema, acknowledgementConfig, originalAcknowledgementConfig: acknowledgementConfig, approvalSignatureConfig, originalApprovalSignatureConfig: approvalSignatureConfig, documentNumberRule, originalDocumentNumberRule: documentNumberRule, formData, originalFormData: formData, sourceModule: issue?.source_module ?? 'custom', originalSourceModule: issue?.source_module ?? 'custom', sourceRecordId: issue?.source_record_id ?? '', originalSourceRecordId: issue?.source_record_id ?? '', issued_at: issue?.issued_at ?? null } : current);
  }, [editor, issuesQuery.data, templatesQuery.data]);
  ...
  const importGoogleDoc = useMutation({
    mutationFn: () => apiFetch<FormTemplate>('/api/v1/forms/templates/import-google-doc', { method: 'POST', body: JSON.stringify({ ...googleDocImport, name: googleDocImport.name || undefined, description: googleDocImport.description || undefined, moduleKey: googleDocImport.moduleKey || null }) }),
    onSuccess: (template) => {
      void queryClient.invalidateQueries({ queryKey: ['form-templates'] });
      setShowImportGoogleDoc(false);
      setGoogleDocImport({ source: '', name: '', description: '', category: 'IT Support', moduleKey: '' });
      setTab('templates');
      openTemplate(template);
    },
  });
  const createIssue = useMutation({
    mutationFn: () => apiFetch<IssueForm>('/api/v1/forms/issues', { method: 'POST', body: JSON.stringify({ title: newIssue.title, templateId: newIssue.templateId, ticketId: newIssue.ticketId || null, sourceModule: newIssue.sourceModule, sourceRecordId: newIssue.sourceRecordId || newIssue.ticketId || null }) }),
    onSuccess: (issue) => {
  ...
  const saveEditor = useMutation({
    mutationFn: async () => {
      ...
      return editor.kind === 'template'
        ? apiFetch<FormTemplate>(`/api/v1/forms/templates/${editor.id}`, { method: 'PATCH', body: JSON.stringify({ name: editor.title, description: editor.description, category: editor.category, contentHtml: editor.html, fieldSchema: editor.fieldSchema ?? [], acknowledgementConfig: editor.acknowledgementConfig ?? defaultAcknowledgementConfig, approvalSignatureConfig: editor.approvalSignatureConfig ?? defaultApprovalSignatureConfig, documentNumberRule: editor.documentNumberRule ?? defaultDocumentNumberRule, moduleKey: editor.moduleKey || null }) })
        : apiFetch<IssueForm>(`/api/v1/forms/issues/${editor.id}`, { method: 'PATCH', body: JSON.stringify({ title: editor.title, contentHtml: editor.html, formData: editor.formData ?? {}, sourceModule: editor.sourceModule ?? 'custom', sourceRecordId: editor.sourceRecordId || null }) });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: editor?.kind === 'template' ? ['form-templates'] : ['issue-forms'] });
      setEditor((current) => current ? { ...current, originalHtml: current.html, originalTitle: current.title, originalDescription: current.description, originalCategory: current.category, originalModuleKey: current.moduleKey, originalFieldSchema: current.fieldSchema ?? [], originalAcknowledgementConfig: current.acknowledgementConfig ?? defaultAcknowledgementConfig, originalApprovalSignatureConfig: current.approvalSignatureConfig ?? defaultApprovalSignatureConfig, originalDocumentNumberRule: current.documentNumberRule ?? defaultDocumentNumberRule, originalFormData: current.formData ?? {}, originalSourceModule: current.sourceModule, originalSourceRecordId: current.sourceRecordId } : current);
    },
  });
  const publishTemplate = useMutation({
    mutationFn: async () => {
      if (!editor || editor.kind !== 'template') throw new Error('ไม่ได้เลือก Template');
      if (editor.html !== editor.originalHtml || editor.title !== editor.originalTitle || editor.description !== editor.originalDescription || editor.category !== editor.originalCategory || editor.moduleKey !== editor.originalModuleKey || JSON.stringify(editor.fieldSchema ?? []) !== JSON.stringify(editor.originalFieldSchema ?? []) || JSON.stringify(editor.acknowledgementConfig ?? defaultAcknowledgementConfig) !== JSON.stringify(editor.originalAcknowledgementConfig ?? defaultAcknowledgementConfig) || JSON.stringify(editor.approvalSignatureConfig ?? defaultApprovalSignatureConfig) !== JSON.stringify(editor.originalApprovalSignatureConfig ?? defaultApprovalSignatureConfig) || JSON.stringify(editor.documentNumberRule ?? defaultDocumentNumberRule) !== JSON.stringify(editor.originalDocumentNumberRule ?? defaultDocumentNumberRule)) {
        await apiFetch<FormTemplate>(`/api/v1/forms/templates/${editor.id}`, { method: 'PATCH', body: JSON.stringify({ name: editor.title, description: editor.description, category: editor.category, contentHtml: editor.html, fieldSchema: editor.fieldSchema ?? [], acknowledgementConfig: editor.acknowledgementConfig ?? defaultAcknowledgementConfig, approvalSignatureConfig: editor.approvalSignatureConfig ?? defaultApprovalSignatureConfig, documentNumberRule: editor.documentNumberRule ?? defaultDocumentNumberRule, moduleKey: editor.moduleKey || null }) });
      }
    ...
    onSuccess: (template) => {
      void queryClient.invalidateQueries({ queryKey: ['form-templates'] });
      setEditor((current) => current ? { ...current, code: `${template.template_code} · v${template.current_version}`, originalHtml: current.html, originalTitle: current.title, originalDescription: current.description, originalCategory: current.category, originalModuleKey: current.moduleKey, originalFieldSchema: current.fieldSchema ?? [], originalAcknowledgementConfig: current.acknowledgementConfig ?? defaultAcknowledgementConfig, originalApprovalSignatureConfig: current.approvalSignatureConfig ?? defaultApprovalSignatureConfig, originalDocumentNumberRule: current.documentNumberRule ?? defaultDocumentNumberRule } : current);
    },
  });
  ...
  const compareTemplate = useMutation({
    mutationFn: async () => {
      if (!editor || editor.kind !== 'template') throw new Error('เลือก Template ก่อน');
      const version = Number((editor.code.match(/v(\d+)$/) ?? [])[1] ?? 1);
      if (version < 2) throw new Error('Template นี้มีเพียงเวอร์ชันเดียว');
      return apiFetch<{ from: { version: number; content_html: string }; to: { version: number; content_html: string }; changed: Record<string, boolean> }>(`/api/v1/forms/templates/${editor.id}/compare?from=${version - 1}&to=${version}`);
    },
    onSuccess: (result) => setCompareResult(result),
  });
  const issueAndExport = useMutation({
    mutationFn: async () => {
      if (!editor || editor.kind !== 'issue') throw new Error('เลือกแบบฟอร์มงานก่อน');
      const issued = await apiFetch<{ issued_at?: string }>(`/api/v1/forms/issues/${editor.id}/issue`, { method: 'POST' });
      setEditor((current) => current ? { ...current, issued_at: issued.issued_at ?? new Date().toISOString() } : current);
      return apiFetch<{ filename: string; pdfBase64: string }>(`/api/v1/forms/issues/${editor.id}/exports/pdf`, { method: 'POST' });
    },
    onSuccess: (result) => { downloadPdfBase64(result.pdfBase64, result.filename); void queryClient.invalidateQueries({ queryKey: ['issue-forms'] }); setEditor((current) => current ? { ...current, issued_at: new Date().toISOString() } : current); },
  });
  const exportPdf = useMutation({
    ...
    onSuccess: (result) => downloadPdfBase64(result.pdfBase64, result.filename),
  });
  const isDirty = editor ? editor.html !== editor.originalHtml || editor.title !== editor.originalTitle || editor.description !== editor.originalDescription || editor.category !== editor.originalCategory || editor.moduleKey !== editor.originalModuleKey || JSON.stringify(editor.fieldSchema ?? []) !== JSON.stringify(editor.originalFieldSchema ?? []) || JSON.stringify(editor.formData ?? {}) !== JSON.stringify(editor.originalFormData ?? {}) || editor.sourceModule !== editor.originalSourceModule || editor.sourceRecordId !== editor.originalSourceRecordId : false;
  ...
  if (editor) {
    return <div className="space-y-3" data-testid="form-studio-editor">
      <div className="flex flex-wrap justify-end gap-2">
        {editor.kind === 'template' && <Button size="sm" variant="outline" isLoading={compareTemplate.isPending} onClick={() => compareTemplate.mutate()}><GitCompareArrows className="h-4 w-4" />Compare Version</Button>}
        {editor.kind === 'issue' && canManage && <Button size="sm" variant="secondary" isLoading={issueAndExport.isPending} disabled={Boolean(editor.issued_at)} onClick={() => issueAndExport.mutate()}><FileDown className="h-4 w-4" />ออกเอกสารและ Export PDF</Button>}
        {editor.kind === 'issue' && canExport && Boolean(editor.issued_at) && <Button size="sm" variant="outline" isLoading={exportPdf.isPending} onClick={() => exportPdf.mutate()}><FileDown className="h-4 w-4" />Export PDF</Button>}
      </div>
      {compareResult && <VersionCompareModal result={compareResult} onClose={() => setCompareResult(undefined)} />}
      ...
      {editor.kind === 'template' && <TemplateGovernancePanel acknowledgementConfig={editor.acknowledgementConfig ?? defaultAcknowledgementConfig} approvalSignatureConfig={editor.approvalSignatureConfig ?? defaultApprovalSignatureConfig} documentNumberRule={editor.documentNumberRule ?? defaultDocumentNumberRule} readOnly={!canManage} onChange={({ acknowledgementConfig, approvalSignatureConfig, documentNumberRule }) => setEditor((current) => current ? { ...current, acknowledgementConfig, approvalSignatureConfig, documentNumberRule } : current)} />}
      <div className={editor.kind === 'issue' && editor.vendorResponse?.submittedAt ? 'grid gap-3 xl:grid-cols-[minmax(0,1fr)_340px]' : ''}>
        {editor.kind === 'template' && <FieldSchemaBuilder value={editor.fieldSchema ?? []} onChange={(fieldSchema) => setEditor((current) => current ? { ...current, fieldSchema } : current)} readOnly={!canManage} />}
        {editor.kind === 'issue' && <StructuredFieldPanel schema={editor.fieldSchema ?? []} values={editor.formData ?? {}} onChange={(formData) => setEditor((current) => current ? { ...current, formData } : current)} readOnly={!canManage || editor.status === 'Closed' || Boolean(editor.issued_at)} />}
        {editor.kind === 'issue' && <GenericLinkPanel module={editor.sourceModule ?? 'custom'} recordId={editor.sourceRecordId ?? ''} sources={referencesQuery.data?.sources} onChange={(sourceModule, sourceRecordId) => setEditor((current) => current ? { ...current, sourceModule, sourceRecordId } : current)} readOnly={!canManage || Boolean(editor.issued_at)} />}
        <WordLikeEditor value={editor.html} onChange={(html) => setEditor((current) => current ? { ...current, html } : current)} fileName={`${editor.code}-${editor.title}`} readOnly={!canManage || editor.status === 'Closed' || Boolean(editor.issued_at)} />
        {editor.kind === 'issue' && editor.vendorResponse?.submittedAt && <VendorResponsePanel response={editor.vendorResponse} />}
      </div>
      {editor.kind === 'issue' && <FormEvidencePanel issueId={editor.id} issuedAt={editor.issued_at} canManage={canManage} onSaved={() => void queryClient.invalidateQueries({ queryKey: ['issue-forms'] })} />}
  ...
  return <div className="space-y-5" data-testid="form-studio-page">
    <div className="flex flex-wrap items-start justify-between gap-3"><PageTitle eyebrow="บริการและกระบวนการ IT / แบบฟอร์มงาน" title="Form Studio" description="จัดการแบบฟอร์มหลักตามประเภทงาน เลือกโมดูลให้ระบบนำไปใช้กับรายการใหม่อัตโนมัติ พร้อมดาวน์โหลด Word" />{canManage && <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setShowImportGoogleDoc(true)}><ExternalLink className="h-4 w-4" />นำเข้า Google Docs</Button><Button variant="outline" onClick={() => setShowNewTemplate(true)}><FilePlus2 className="h-4 w-4" />สร้างแบบฟอร์มหลัก</Button><Button onClick={() => setShowNewIssue(true)}><Plus className="h-4 w-4" />สร้างแบบฟอร์มงาน</Button></div>}</div>
    ...
    {showNewIssue && <FormModal title="สร้างแบบฟอร์มงาน" description="คัดลอก Template เวอร์ชันปัจจุบันมาเป็นเอกสารงานที่แก้ไขได้อิสระ" size="lg" onClose={() => setShowNewIssue(false)} footer={<><Button variant="ghost" onClick={() => setShowNewIssue(false)}>ยกเลิก</Button><Button isLoading={createIssue.isPending} disabled={!newIssue.title.trim() || !newIssue.templateId} onClick={() => createIssue.mutate()}><Plus className="h-4 w-4" />สร้างแบบฟอร์มงาน</Button></>}><div className="grid gap-4"><label className="text-sm font-semibold">ชื่อเรื่อง<input value={newIssue.title} onChange={(event) => setNewIssue({ ...newIssue, title: event.target.value })} className={fieldClass} /></label><label className="text-sm font-semibold">Template<select value={newIssue.templateId} onChange={(event) => setNewIssue({ ...newIssue, templateId: event.target.value })} className={fieldClass}><option value="">— เลือก Template —</option>{templatesQuery.data?.filter((item) => item.status !== 'Archived').map((template) => <option key={template.id} value={template.id}>{template.template_code} · {template.name} · v{template.current_version}</option>)}</select></label><GenericLinkPanel module={newIssue.sourceModule} recordId={newIssue.sourceRecordId} sources={referencesQuery.data?.sources} onChange={(sourceModule, sourceRecordId) => setNewIssue({ ...newIssue, sourceModule, sourceRecordId, ticketId: sourceModule === 'ticket' ? sourceRecordId : '' })} /></div></FormModal>}
    {showImportGoogleDoc && <FormModal title="นำเข้า Google Docs" description="วางลิงก์ Google Docs แล้วระบบจะอ่านเนื้อหาเข้ามาเป็น Template โดยไม่ดาวน์โหลดไฟล์ลงเครื่อง" size="lg" onClose={() => setShowImportGoogleDoc(false)} footer={<><Button variant="ghost" onClick={() => setShowImportGoogleDoc(false)}>ยกเลิก</Button><Button isLoading={importGoogleDoc.isPending} disabled={!googleDocImport.source.trim()} onClick={() => importGoogleDoc.mutate()}><ExternalLink className="h-4 w-4" />นำเข้าและเปิด Editor</Button></>}>
      <div className="grid gap-4 md:grid-cols-2">
        <label className="text-sm font-semibold md:col-span-2">ลิงก์ Google Docs หรือรหัสเอกสาร<input value={googleDocImport.source} onChange={(event) => setGoogleDocImport({ ...googleDocImport, source: event.target.value })} placeholder="https://docs.google.com/document/d/..." className={fieldClass} autoFocus /></label>
        <p className="text-xs text-slate-500 md:col-span-2">เอกสารต้องแชร์ให้บัญชี Service Account ของระบบมีสิทธิ์ Viewer เป็นอย่างน้อย และควรใช้เอกสาร Google Docs ไม่ใช่ไฟล์ Word ที่อัปโหลดไว้</p>
        <label className="text-sm font-semibold">ชื่อ Template (ไม่บังคับ)<input value={googleDocImport.name} onChange={(event) => setGoogleDocImport({ ...googleDocImport, name: event.target.value })} placeholder="ใช้ชื่อเอกสาร Google Docs หากเว้นว่าง" className={fieldClass} /></label>
        <label className="text-sm font-semibold">หมวดหมู่<input value={googleDocImport.category} onChange={(event) => setGoogleDocImport({ ...googleDocImport, category: event.target.value })} className={fieldClass} /></label>
        <label className="text-sm font-semibold">ใช้เป็นแบบฟอร์มของโมดูล<select value={googleDocImport.moduleKey} onChange={(event) => setGoogleDocImport({ ...googleDocImport, moduleKey: event.target.value as FormModuleKey | '' })} className={fieldClass}><option value="">— Template กลาง / ยังไม่ผูกโมดูล —</option>{FORM_MODULES.map((module) => <option key={module.key} value={module.key}>{module.label}</option>)}</select></label>
        <label className="text-sm font-semibold md:col-span-2">คำอธิบาย (ไม่บังคับ)<textarea rows={3} value={googleDocImport.description} onChange={(event) => setGoogleDocImport({ ...googleDocImport, description: event.target.value })} placeholder="ระบุรายละเอียดเพิ่มเติมของ Template" className={fieldClass} /></label>
      </div>
    </FormModal>}
...
function IssueLinks({ issue, onOpenTemplate }: { issue: IssueForm; onOpenTemplate: (templateId: string) => void }) {
  ...
  const genericSource = issue.source_module && issue.source_module !== 'ticket' && issue.source_record_id ? `${issue.source_module} · ${issue.source_record_id}` : null;
  return <>
    {template
    ...
    {ticket
      ...
      : <p className="mt-0.5 text-xs text-slate-400">{genericSource ? `Generic Link: ${genericSource}` : 'ไม่ผูก Source Record'}</p>}
  </>;
}
...
function VersionCompareModal({ result, onClose }: { result: { from: { version: number; content_html: string }; to: { version: number; content_html: string }; changed: Record<string, boolean> }; onClose: () => void }) {
  return <Modal title="Compare Version" description={`v${result.from.version} → v${result.to.version}`} onClose={onClose} footer={<Button onClick={onClose}>ปิด</Button>}><div className="space-y-3 text-sm"><p className="rounded-lg bg-slate-50 p-3 dark:bg-slate-900">เปลี่ยนแปลง: {Object.entries(result.changed).filter(([, changed]) => changed).map(([key]) => key).join(', ') || 'ไม่มีความแตกต่าง'}</p><div className="grid gap-3 md:grid-cols-2"><div className="max-h-72 overflow-auto rounded-lg border p-3"><p className="mb-2 font-bold">v{result.from.version}</p><div dangerouslySetInnerHTML={{ __html: result.from.content_html }} /></div><div className="max-h-72 overflow-auto rounded-lg border p-3"><p className="mb-2 font-bold">v{result.to.version}</p><div dangerouslySetInnerHTML={{ __html: result.to.content_html }} /></div></div></div></Modal>;
}
